...
Public Class frmImportWESMTransAllocSummaryFlatFile
    Private _WTSummaryHelper As New ImportWESMTransSummaryPrelimHelper
    ...
    Private Async Sub btnImport_Click(sender As Object, e As EventArgs) Handles btnImport.Click
        Try
            ...
            If rb_Prelim.Checked = True Then
                ...
                If Not fnameSplit(2).ToUpper.Equals("WP") And Not fnameSplit(2).ToUpper.Equals("WAC") And Not fnameSplit(2).ToUpper.Equals("WAD") Then
                    MessageBox.Show("Invalid filename! Please choose another file.", "Invalid", MessageBoxButtons.OK, MessageBoxIcon.Error)
                    Exit Sub
                End If
            Else
            ...
            Await Task.Run(Sub() Me._WTSummaryHelper.ReadFlatFileNew(fileBillingPd, fileSettlementRun, wesmBilltransType, EnumFileType.Energy, pathFolder, fileName, remarks, transDate, dueDate, progressIndicator))
            ...
            For Each item In Me._WTSummaryHelper.NewListWBAllocCoverSummary.Select(Function(x) x.StlID).Distinct.ToList()
                Dim ParentID = item
            ...
            If UCase(fnameSplit2(0).ToString) = "INV" Then
                ...
                Dim Vat = (From x In _WTSummaryHelper.NewListWBAllocCoverSummary
                ...
                Dim AP As Decimal = (From x In _WTSummaryHelper.NewListWBAllocCoverSummary
                                     Where x.NetSale > 0
                                     Select x.NetSale).Sum()

                Dim AR As Decimal = (From x In _WTSummaryHelper.NewListWBAllocCoverSummary
                ...
                Me.chckList.Items.Clear()
                For Each item In _WTSummaryHelper.NewListWBAllocCoverSummary.Select(Function(y) y.StlID).OrderBy(Function(x) x).Distinct.ToList
    ...
    Private Sub btnDownload_Click(sender As Object, e As EventArgs) Handles btnDownload.Click
        Try
            ...
            For cnt As Integer = 0 To Me.chckList.CheckedItems.Count - 1
                ...
                listWESMTransSummary = _WTSummaryHelper.NewListWBAllocCoverSummary.Where(Function(x) x.StlID = getParticpantInfo.IDNumber).Select(Function(z) z).ToList
                ...
                Parallel.ForEach(listWESMTransSummary, Sub(WESMTransSummary)
                                                           ...
                                                           Dim ds As DataSet = _WTSummaryHelper.PrintWESMTransactionSummary(New DSReport.WESMBillTransCoverSummaryDataTable, New DSReport.WESMBillTransAllocDetailsDataTable,
                                                           ...
                                                           dest.DiskFileName = filePath & "\" & participant & "_" & WESMTransSummary.TransactionNo & "_WTA.pdf"
                                                           ...
                                                           If Me.rb_Prelim.Checked = True Then
                                                               Dim expReport = New RPTWESMBillTransCoverSummaryPrelim
    ...
    Private Async Sub btnSave_Click(sender As Object, e As EventArgs) Handles btnSave.Click
        Try
            ...
            If ans = MsgBoxResult.Yes Then
                ...
                _WTSummaryHelper.CreateWESMBill()
                Dim dueDate As Date = CDate(FormatDateTime(Me.dtDueDate.Value, DateFormat.ShortDate))
                Dim itemJV As JournalVoucher
                itemJV = Me.GenerateJournalVoucherForEnergy(_WTSummaryHelper.NewListWESMBill, 0)

                Dim itemGP = Me.GenerateGPPosted(getCalendarBP, fileSettlementRun, dueDate,
                                                     fileChargeType, itemJV)

                ProgressThread.Show("Please wait while saving.")
                Await Task.Run(Sub() _WTSummaryHelper.SaveUplodedWESMBill(getCalendarBP, fileSettlementRun, fileChargeType, EnumFileType.Energy, itemJV, itemGP, CountWESMBillExist, progressIndicator, cts.Token))
                ProgressThread.Close()
    ...
    Private Sub cmd_ExportInExcel_Click(sender As Object, e As EventArgs) Handles btnExportWTASummary.Click
        ...
        Try
            For Each item In _WTSummaryHelper.NewListWBAllocCoverSummary
...
#Region "Method and Functions For Generation of Collection And Payment Report on MS-Excel"
    ...
    Public Sub CreateOutstandingSummaryReport(ByVal SavingPathName As String)
        ...
        ReDim WESMBillSummaryHeader(1, 19)
        ...
        If _WTSummaryHelper.NewListWBAllocCoverSummary.Count <> 0 Then
    ...
    Private Function GenerateWESMBillArray() As Object(,)
        ...
        For Each item In getDistinctTransNo
            ...
            Dim getWESMAllocTransSummary As WESMBillAllocCoverSummary = (From x In _WTSummaryHelper.NewListWBAllocCoverSummary Where x.TransactionNo = item Select x).FirstOrDefault
    ...
    Private Function GenerateWESMTransSummaryArray() As Object(,)
        ...
        Dim WBSArr As Object(,) = New Object(,) {}
        ReDim WBSArr(RowCount, 19)

        Dim getDistinctTransNo As List(Of String) = (From x In _WTSummaryHelper.NewListWBAllocCoverSummary Select x.TransactionNo Distinct Order By TransactionNo).ToList
    ...
    Private Function GenerateWESMTransDetailsArray() As Object(,)
        ...
        Dim getDistinctTransNo As List(Of String) = (From x In listWESMBill Select x.InvoiceNumber Distinct Order By InvoiceNumber).ToList

        For Each item In getDistinctTransNo
            Dim getWESMAllocTransSummary As WESMBillAllocCoverSummary = (From x In _WTSummaryHelper.NewListWBAllocCoverSummary Where x.TransactionNo = item Select x).FirstOrDefault
            RowCount += getWESMAllocTransSummary.ListWBAllocDisDetails.Count
        Next
        ...
        For Each item In getDistinctTransNo
            Dim getWESMAllocTransSummary As WESMBillAllocCoverSummary = (From x In _WTSummaryHelper.NewListWBAllocCoverSummary Where x.TransactionNo = item Select x).FirstOrDefault
            Dim getAMParticipantInfo = (From x In listParticipantInfo Where x.IDNumber = getWESMAllocTransSummary.StlID Select x).FirstOrDefault
            For Each dtl In getWESMAllocTransSummary.ListWBAllocDisDetails
                WBSArr(RowIndex, 0) = dtl.STLID
                WBSArr(RowIndex, 1) = dtl.BillingID
                WBSArr(RowIndex, 2) = dtl.FacilityType
                WBSArr(RowIndex, 3) = dtl.WHTTag
                WBSArr(RowIndex, 4) = dtl.NonVatableTag
                WBSArr(RowIndex, 5) = dtl.ZeroRatedTag
                WBSArr(RowIndex, 6) = dtl.VatableSales
                WBSArr(RowIndex, 7) = dtl.ZeroRatedSales
                WBSArr(RowIndex, 8) = dtl.ZeroRatedEcoZoneSales
                WBSArr(RowIndex, 9) = dtl.VatOnSales
                WBSArr(RowIndex, 10) = dtl.VatablePurchases
                WBSArr(RowIndex, 11) = dtl.ZeroRatedPurchases
                WBSArr(RowIndex, 12) = dtl.ZeroRatedEcoZonePurchases
                WBSArr(RowIndex, 13) = dtl.VatOnPurchases
                WBSArr(RowIndex, 14) = dtl.EWT
                WBSArr(RowIndex, 15) = getWESMAllocTransSummary.TransactionNo
                RowIndex += 1
            Next
        Next
        ret = WBSArr
    ...
    Private Sub btnExportperSTLID_Click(sender As Object, e As EventArgs) Handles btnExportperSTLID.Click
        ...
        Try
            ...
            For cnt As Integer = 0 To Me.chckList.CheckedItems.Count - 1
                ...
                UpdateProgress(newProgress)
                Parallel.ForEach(listWESMTransSummary, Sub(WESMTransSummary)
                                                           CreateWTAperParticipants(TargetPath, getParticpantInfo, WESMTransSummary)
                                                       End Sub)
            Next
