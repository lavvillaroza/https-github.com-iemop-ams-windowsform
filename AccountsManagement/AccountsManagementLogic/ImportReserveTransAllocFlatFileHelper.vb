...
Public Class ImportReserveTransAllocFlatFileHelper
    Public Sub New()
    ...
    Public Sub ReadFlatFileNew(ByVal bpNo As Integer, ByVal stlRun As String, ByVal wesmBilltransType As String, ByVal fileType As EnumFileType, ByVal pathFile As String,
        ...
        Try
            ...
            Dim columnHeaders As String() = {"STL_ID", "BILLING_ID", "FACILITY_TYPE", "WHT_TAG", "ITH_TAG", "NON_VATABLE_TAG",
                                              ...
                                              "EWT", "NSS", "GMR", "MF_RATE", "SELLER/BUYER", "SPOT_QTY", "GENX_AMT", "INVOICE_ID", "RESERVE_CATEGORY", "AS_NONPAYMENT", "RESERVE_AMOUNT_ADJ", "REGION"}
            ...
            For Each row As DataRow In dtDataTable.Rows
                ...
                ElseIf Not CStr(row.Item("FACILITY_TYPE").ToUpper.ToString).Equals("ASP") And Not CStr(row.Item("FACILITY_TYPE").ToUpper.ToString).Equals("ASB") Then
                    Throw New ApplicationException("Error in row " & rowIndx & "." & "Invalid FACILITY_TYPE found in File (" & CStr(row.Item("FACILITY_TYPE").ToUpper.ToString) & ")")
                End If
                ...
                If IsDBNull(row.Item("RESERVE_CATEGORY")) Then
                    Throw New ApplicationException("Error in row " & rowIndx & "." & "Invalid RESERVE_CATEGORY found in File")
                End If
                wbFileOutputItem.ReserveCategory = CStr(row.Item("RESERVE_CATEGORY"))

                If Not IsDBNull(row.Item("AS_NONPAYMENT")) And Len(row.Item("AS_NONPAYMENT")) <> 0 Then
                    If Not IsNumeric(row.Item("AS_NONPAYMENT")) Then
                        Throw New ApplicationException("Error in row " & rowIndx & "." & "Invalid AS_NONPAYMENT found in File")
                    End If
                    wbFileOutputItem.ASNonPayment = CDec(row.Item("AS_NONPAYMENT"))
                    If CDec(row.Item("AS_NONPAYMENT")) <> 0 Then
                        Dim x0 = 0
                    End If
                Else
                    wbFileOutputItem.ASNonPayment = 0
                End If

                If Not IsDBNull(row.Item("RESERVE_AMOUNT_ADJ")) And Len(row.Item("RESERVE_AMOUNT_ADJ")) <> 0 Then
                    If Not IsNumeric(row.Item("RESERVE_AMOUNT_ADJ")) Then
                        Throw New ApplicationException("Error in row " & rowIndx & "." & "Invalid RESERVE_AMOUNT_ADJ found in File")
                    End If
                    wbFileOutputItem.ReserveAmountAdj = CDec(row.Item("RESERVE_AMOUNT_ADJ"))
                Else
                    wbFileOutputItem.ReserveAmountAdj = 0
                End If

                If IsDBNull(row.Item("REGION")) Then
                    Throw New ApplicationException("Error in row " & rowIndx & "." & "Invalid REGION found in File")
                End If
                wbFileOutputItem.Region = CStr(row.Item("REGION"))
    ...
    Private Sub GenerateWESMBills(ByVal bpNo As Integer, ByVal stlRun As String, ByVal wesmBilltransType As String, ByVal fileType As EnumFileType,
        ...
        Dim groupListSellerBuyerInformation As List(Of SellerBuyerInformation) = listWBFileOutput.
                                                                                GroupBy(Function(sbInfo) New With {
                                                                                                          ...
                                                                                                          Key sbInfo.NetSellerBuyerTag, Key sbInfo.ReserveCategory,
                                                                                                          Key sbInfo.Region}).
                                                                                Select(Function(x) New SellerBuyerInformation With {
                                                                                       ...
                                                                                       .NetSellerBuyerTag = x.Key.NetSellerBuyerTag, .ReserveCategory = x.Key.ReserveCategory,
                                                                                       .Region = x.Key.Region}).OrderBy(Function(z) z.BillingId).
                                                                                ToList
        ...
        For Each item In getSellerWTACoverSummary
            If item.BillingId = "1590ECASP" Then
                Dim x0 = 0
            End If
            summaryNo += 1
            ...
            Dim getWESMBillFileOutputPerBillingId As List(Of WESMBillFileOutput) = listWBFileOutput.
                                                                                   Where(Function(x) x.BillingId = item.BillingId _
                                                                                   And x.InvoiceID = item.InvoiceID _
                                                                                   And x.StlID = item.StlID _
                                                                                   And x.NetSellerBuyerTag.ToUpper = "SELLER" _
                                                                                   And x.ReserveCategory = item.ReserveCategory _
                                                                                   And x.Region = item.Region).
            ...
            With wesmAllocCoverSummary
                ...
                .ReserveCategory = getWESMBillFileOutputPerBillingId.Select(Function(x) x.ReserveCategory).First
                .ASNonPayment = getWESMBillFileOutputPerBillingId.Select(Function(x) x.ASNonPayment).First
                .ReserveAmountAdj = getWESMBillFileOutputPerBillingId.Select(Function(x) x.ReserveAmountAdj).First
                .Region = getWESMBillFileOutputPerBillingId.Select(Function(x) x.Region).First
        ...
        For Each item In getBuyerWTACoverSummary
            ...
            Dim getWESMBillFileOutputPerBillingId As List(Of WESMBillFileOutput) = listWBFileOutput.
                                                                                   ...
                                                                                   And x.InvoiceID = item.InvoiceID _
                                                                                   And x.StlID = item.StlID _
                                                                                   And x.NetSellerBuyerTag.ToUpper = "BUYER" _
                                                                                   And x.ReserveCategory = item.ReserveCategory _
                                                                                   And x.Region = item.Region).
                                                                                   Select(Function(z) z).ToList
            For Each dtl In getWESMBillFileOutputPerBillingId
    ...
    Public Sub CreateWESMBill()
        ...
        For Each item In Me._NewListWBAllocCoverSummary
            ...
            With wesmBillItemEnergy
                ...
                .SettlementRun = item.STLRun.Replace("F", "R")
                .IDNumber = item.StlID
        ...
        Try
            ...
            For Each item In Me.NewListWBAllocCoverSummary
                ...
                SQL = "INSERT INTO AM_WESM_ALLOC_COVER_SUMMARY(SUMMARY_ID,STL_RUN,GROUP_ID,BILLING_PERIOD,STL_ID,BILLING_ID,NON_VATABLE_TAG,ZERO_RATED_TAG,WHT_TAG,ITH_TAG,NET_SELLER_BUYER_TAG," & vbNewLine _
                                                               ...
                                                               & "ZERO_RATED_ECOZONE_PURCHASES, REGION, AS_NONPAYMENT, RESERVE_AMOUNT_ADJ, RESERVE_CATEGORY)" & vbNewLine _
                      & "SELECT " & summaryNo & ", '" & item.STLRun & "', ''," & item.BillingPeriod & ", '" & item.StlID & "', '" & item.BillingID & "', '" & item.NonVatableTag & "', '" & item.ZeroRatedTag & "', " & vbNewLine _
                                  ...
                                  & ", 0, " & item.GMR & ", " & item.MarketFeesRate & ", '" & item.Remarks & "', 'FLATFILE'," & item.SpotQty & "," & item.GenXAmount & "," & item.ZeroRatedEcoZonePurchases & vbNewLine _
                                  & ", '" & item.Region & "', " & item.ASNonPayment & ", " & item.ReserveAmountAdj & ",'" & item.ReserveCategory & "' FROM DUAL"
