...
Public Class frmImportReservedFromCRSS
    Private impRESACRSSHelper As New ImportReservedFromCRSSDBHelper
    ...
    Private Sub frmImportReservedFromCRSS_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Private Sub RefreshDueDateList()
        ...
        For Each item In impRESACRSSHelper.NewRTADueDateList
    ...
    Private Async Sub btnImport_Click(sender As Object, e As EventArgs) Handles btnImport.Click
        ...
        Try
            ...
            For index As Integer = 0 To Me.dgv_WTAList.RowCount - 1
                With Me.dgv_WTAList.Rows(index)
                    If CBool(.Cells("colChckBox").Value) Then
                        ...
                        Dim isBillPosted = impRESACRSSHelper._WBillHelper.IsWESMBillPosted(billingPeriod, stlRun, EnumChargeType.E)
            ...
            For index As Integer = 0 To Me.dgv_WTAList.RowCount - 1
                With Me.dgv_WTAList.Rows(index)
                    If CBool(.Cells("colChckBox").Value) Then
                        ...
                        CountWESMBillExist = impRESACRSSHelper._WBillHelper.GetWESMBillCount(billingPeriod, stlRun, EnumChargeType.E)
            ...
            Me.listOfSignatories = impRESACRSSHelper._WBillHelper.GetSignatories()
            ...
            For index As Integer = 0 To Me.dgv_WTAList.RowCount - 1
                With Me.dgv_WTAList.Rows(index)
                    If CBool(.Cells("colChckBox").Value) Then
                        ...
                        Dim getWTACoverSummaryItem As ImportRTAFromCRSS = impRESACRSSHelper.newImportResFromCRSSList.Where(Function(x) x.BillingPeriod = billingPeriod And x.SettlementRun = stlRun And x.GroupID = groupID).First()

                        Dim itemJV As JournalVoucher = Me.GenerateJournalVoucherForEnergy(getWTACoverSummaryItem.WESMBillList, 0)
                        Dim getCalendarBP As CalendarBillingPeriod = (From x In impRESACRSSHelper._WBillHelper.GetCalendarBP() Where x.BillingPeriod = billingPeriod Select x).FirstOrDefault
                        ...
                        Await Task.Run(Sub() impRESACRSSHelper.SaveUplodedWESMBill(getCalendarBP, stlRun, groupID, EnumChargeType.E, 0, itemJV, itemGP, CountWESMBillExist, progressIndicator, cts.Token))
...
#Region "Functions/Methods"
    ...
    Private Sub cmb_DueDate_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cmb_DueDate.SelectedIndexChanged
        Try
            ...
            impRESACRSSHelper.FillTheDGV(getSelectedDueDate)
            Me.dgv_WTAList.Rows.Clear()
            For Each item In impRESACRSSHelper.newImportResFromCRSSList
        ...
        If (e.ColumnIndex = 0 AndAlso e.RowIndex >= 0) Then
            ...
            Try
                ...
                Else
                    ...
                    Me.impRESACRSSHelper.FetchSelectedData(rowBP, rowSTLRun, getSelectedDueDate, rowGroupID)
                    Dim getSelectedWTAItem As ImportRTAFromCRSS = Me.impRESACRSSHelper.newImportResFromCRSSList.Where(Function(x) x.BillingPeriod = rowBP And x.SettlementRun = rowSTLRun And x.GroupID = rowGroupID).FirstOrDefault
    ...
    Private Sub btnRefresh_Click(sender As Object, e As EventArgs) Handles btnRefresh.Click
        Me.RefreshDueDateList()
        Me.impRESACRSSHelper = New ImportReservedFromCRSSDBHelper
    End Sub
